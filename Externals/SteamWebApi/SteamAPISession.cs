...
namespace SteamWebAPI
{
    ...
    public class SteamAPISession
    {
        ...
        {
            String response = steamRequest( "ISteamUser/GetPlayerSummaries/v0002?key=" + accessToken + "&steamids=" + String.Join( ",", steamids.GetRange( 0, Math.Min( steamids.Count, 100 ) ).ToArray() ) );
            ...
            if ( response != null )
            {
                ...
                if ( data["response"] != null )
                {
                    List<User> users = new List<User>();
                        ...
                        foreach (JObject info in data["response"]["players"])
                        {
                            User user = new User();
